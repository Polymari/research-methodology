...
def load_test_queries_with_ground_truth(df_theorems=None):
    ...
    for _, row in df_test.iterrows():
        ...
        paper_mask = (
            df_theorems["paper_id"]
            .astype(str)
            .str.contains(arxiv_id, na=False, regex=False)
        )
        ...
        if thm_number:
            ...
            if len(exact) == 1:
                ...
                strict = exact[exact["name"].str.strip().str.startswith(thm_number)]
    ...
    print(
        f"Ground-truth linking: {n_evaluable}/{len(results)} evaluable, {n_exact} exact theorem matches"
    )
