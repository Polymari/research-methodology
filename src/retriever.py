...
class HNSWRetriever:
    ...
    def add_embeddings(self, embeddings: np.ndarray):
        ...
        self.index.add(embeddings)
        print(f"HNSW index built with {self.index.ntotal} vectors (m={self.m}, efC={self.ef_construction})")
    ...
    def search(self, query_embeddings: np.ndarray, top_k=20, candidate_pool_factor=6):
        ...
        for i in range(len(query_embeddings)):
            ...
            if len(reranked_idx) < top_k:
                ...
                reranked_idx = np.concatenate([reranked_idx, np.full(pad_len, -1, dtype=np.int64)])
                reranked_scores = np.concatenate([reranked_scores, np.zeros(pad_len, dtype=np.float32)])
